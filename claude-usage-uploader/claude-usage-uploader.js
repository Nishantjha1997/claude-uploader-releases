...
function sendPing(name, status, message = '', extra = {}) {
  return new Promise((resolve) => {
    ...
    req.on('error', (err) => { log(`Ping failed: ${err.message}`); resolve(); });
    req.on('timeout', () => { req.destroy(); log(`Ping timed out for ${name}`); resolve(); });
    req.write(data);
    req.end();
  });
}
...
function isUploadDue(cfg, frequency) {
  ...
  const lastUploadStr = cfg.lastUploadDate || cfg.lastUploadWeek;
  if (!lastUploadStr) return true;
  const lastUpload = new Date(lastUploadStr);
  const now = new Date();

  if (frequency === 'daily') {
    const todayMidnight = new Date(now);
    todayMidnight.setHours(0, 0, 0, 0);
    return lastUpload < todayMidnight;
  }
  if (frequency === 'monthly') {
    const monthStart = new Date(now.getFullYear(), now.getMonth(), 1);
    return lastUpload < monthStart;
  }
  // Default: weekly
  return lastUpload < getWeekStart(now);
}

// FIX: use local date string instead of toISOString() (which returns UTC and can store
// the previous day's date for UTC+ timezones, causing a double-upload on the same Monday)
function saveUploadDate(cfg) {
  ...
  const dateStr = `${y}-${m}-${d}`;
  const updated = { ...cfg, lastUploadDate: dateStr, lastUploadWeek: dateStr };
...
async function runGenerateAndUpload(cfg, source) {
  try {
    ...
    await sendPing(cfg.name, 'SUCCESS', msg);
    log(`Upload complete (${source}) for week ${weekOf}`);
  } catch (e) {
